...
public class CollisionScript : MonoBehaviour {
    ...
    private void OnMouseDown()
    {
		...
		distanceX = this.transform.position.x - mousePos.x;
		distanceZ = this.transform.position.z - mousePos.z;
		impactPos[waveNumber - 1].x = mousePos.x;
		impactPos[waveNumber - 1].y = mousePos.z;

		GetComponent<Renderer>().material.SetFloat("_xImpact" + waveNumber, mousePos.x);
		GetComponent<Renderer>().material.SetFloat("_zImpact" + waveNumber, mousePos.z);

		GetComponent<Renderer>().material.SetFloat("_OffsetX" + waveNumber, distanceX / mesh.bounds.size.x * 2.5f);
		GetComponent<Renderer>().material.SetFloat("_OffsetZ" + waveNumber, distanceZ / mesh.bounds.size.z * 2.5f);

		GetComponent<Renderer>().material.SetFloat("_WaveAmplitude" + waveNumber, 20 * magnitudeDivider);
